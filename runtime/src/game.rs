...
decl_storage! {
	trait Store for Module<T: Trait> as GameModule {
		// Cities get(city_of): map T::AccountId => City<T::Balance>;
		Cities get(city_of): map T::AccountId => City;
		// Todo: Remove
		// Resources get(resource_of): map T::AccountId => Resource<T::Balance>;
		// TestEnum: map T::AccountId => ResourceType;
	}
}
...
decl_module! {
	pub struct Module<T: Trait> for enum Call where origin: T::Origin {
		...
		// 		_ => panic!("Invalid terrain coordinates"),
		// 	};
